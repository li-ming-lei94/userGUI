...
//	//指向EX SRAM LCD BUF 
	...
	}else
	{
		...
		if(sramlcdbuf==NULL)return 1;//错误
    printf("sramlcdbuf 申请成功\r\n");	
//		LCD_Scan_Dir(DFT_SCAN_DIR);		//设置扫描方向 
//		LCD_Set_Window(0,spbdev.stabarheight,spbdev.spbwidth,spbdev.spbheight);	
//		printf("enter gui_show_string\r\n");
//		gui_show_string("A",2+20+2,(spbdev.stabarheight-16)/2,64,16,16,RED);//显示运营商名字	
...
//		while(1);	
		spbdev.frame=0;
		gui_fill_rectangle(0,0,lcddev.width,spbdev.spbheight,LGRAY);
		res=ai_load_picfile(spb_bkpic_path_tbl[lcdtype][0],0,0,spbdev.spbwidth,spbdev.spbheight,0);//加载第一页背景图片
		if(res==0)
			res=spb_load_icos(0);		//加载第0帧图标 
		spbdev.frame=1;
		gui_fill_rectangle(0,0,lcddev.width,spbdev.spbheight,LGRAYBLUE);
		if(res==0)
...
{ 
	...
	if(clr)
	{
		gui_fill_rectangle(0,0,lcddev.width,spbdev.stabarheight,LIGHTBLUE);
	...
	//SIM900A信息显示
	spb_gsm_signal_show(2,(spbdev.stabarheight-20)/2,2);//显示信号质量
	gui_fill_rectangle(2+20+2,(spbdev.stabarheight-16)/2,64,16,LIGHTBLUE);
...
//	}else 
	...
	if(gui_phy.memdevflag&(1<<0))
		app_show_mono_icos(24+64+2,(spbdev.stabarheight-20)/2,20,20,(u8*)SPB_SD_ICO,WHITE,LIGHTBLUE);
	else gui_fill_rectangle(24+64+2,0,20,spbdev.stabarheight,LIGHTBLUE);

		//显示CPU使用率
	gui_phy.back_color=LIGHTBLUE;
		
	temp=OSStatTaskCPUUsage;
	//if(OSStatTaskCPUUsage>99)temp=99;//最多显示到99%
