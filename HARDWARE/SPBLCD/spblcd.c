...
{	 
	sramlcdbuf[x+y*spbdev.spbwidth+spbdev.frame*spbdev.spbheight*spbdev.spbwidth]=color;
}
//读取指定位置点的颜色值
//x,y:坐标
//返回值:颜色
...
{
	return sramlcdbuf[x+y*spbdev.spbwidth+spbdev.frame*spbdev.spbheight*spbdev.spbwidth];
} 
//填充颜色
//x,y:起始坐标
//width，height：宽度和高度
...
void slcd_fill(u16 x,u16 y,u16 width,u16 height,u16 color)
{   
	u16 i,j; 
 	for(i=0;i<height;i++)
	{
...
{  
	...
	DMA2_Channel5->CMAR=(u32)&TFTLCD->LCD_RAM;	//目标地址为LCD_RAM
...
{	  
	...
	{     	  
		...
		DMA2_Channel5->CPAR=(u32)(sramlcdbuf+spbdev.spbheight*x+dmatransfered);	//DMA2,改变存储器地址
