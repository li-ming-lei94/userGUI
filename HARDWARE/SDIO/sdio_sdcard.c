...
SD_Error SD_Init(void)
{
  ...
  if((errorstatus==SD_OK)||(SDIO_MULTIMEDIA_CARD==CardType))
	{  		    
		...
		//printf("8.Enter SD_SetDeviceMode()\r\n");
...
{	  
	...
	{
		//printf("enter SD_ReadBlock SD_POLLING_MODE\r\n");
 		INTX_DISABLE();//关闭总中断(POLLING模式,严禁中断打断SDIO读写操作!!!)
		while(!(SDIO->STA&((1<<5)|(1<<1)|(1<<3)|(1<<10)|(1<<9))))//无上溢/CRC/超时/完成(标志)/起始位错误
		{
			//printf("enter SD_ReadBlock Read\r\n");
...
u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt)
{
	...
	u8 n;
	//printf("enter SD_ReadDisk sector=%d cnt=%d\r\n",sector,cnt);
	if(CardType!=SDIO_STD_CAPACITY_SD_CARD_V1_1)lsector<<=9;
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
			//printf("1.enter SD_ReadBlock\rn" );
	...
	}else
	{
		if(cnt==1)
      { 
				//printf("2.enter SD_ReadBlock\rn" );
			 sta=SD_ReadBlock(buf,lsector,512); 
			}   	//单个sector的读操作
		else 
		{
			//printf("3.enter SD_ReadMultiBlocks\rn" );
			sta=SD_ReadMultiBlocks(buf,lsector,512,cnt);//多个sector
		}			
	}
	//printf("exit SD_ReadDisk sector=%d cnt=%d\r\n",sector,cnt);
	return sta;
}
//写SD卡
//buf:写数据缓存区
